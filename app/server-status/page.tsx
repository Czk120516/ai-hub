    ...
    <AppShell title="服务器运行状况" activeNav="server">
      <div className="mx-auto max-w-4xl px-4 py-6">
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-slate-800">服务器运行状况</h2>
            <p className="text-sm text-slate-400">
              开发者专属 · 实时查看服务运行状态与资源占用
            </p>
          </div>
          <button
            onClick={load}
        ...
        {!loading && error && (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-6 py-12 text-center">
            <ShieldAlert className="h-8 w-8 text-amber-500" />
            <p className="text-sm text-amber-700">{error}</p>
            {!isDeveloper && (
              <p className="text-xs text-amber-500">
                当前账号角色为「{user?.role === "banned" ? "已封禁" : "普通用户"}」，仅开发者可访问此页面。
              </p>
            )}
          </div>
        )}
        ...
        {!loading && status && (
          <>
            ...
            <section className="mb-6">
              <h3 className="mb-3 text-sm font-semibold text-slate-700">健康检查</h3>
            ...
            {/* 运行指标 */}
            <section className="mb-6">
              <h3 className="mb-3 text-sm font-semibold text-slate-700">运行指标</h3>
            ...
            {/* 内存占用 */}
            <section className="mb-6">
              <h3 className="mb-3 text-sm font-semibold text-slate-700">内存占用</h3>
              <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            {/* 数据统计 */}
            <section>
              <h3 className="mb-3 text-sm font-semibold text-slate-700">数据统计</h3>
